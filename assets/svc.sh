...
SVC_NAME="redcarbon.agent.service"
SVC_NAME=${SVC_NAME// /_}
SVC_DESCRIPTION="RedCarbon Agent Runner"
...
if [[ -z $TEMPLATE_PATH ]]; then
    TEMPLATE_PATH=./bin/templates/redcarbon.agent.service.template
else
...
if [ ! -f "${TEMPLATE_PATH}" ]; then
    if [[ $IS_CUSTOM_TEMPLATE = 0 ]]; then
        ...
        failed "Service file at '$REDCARBON_AGENT_SERVICE_TEMPLATE' using REDCARBON_AGENT_SERVICE_TEMPLATE env variable is not found"
    fi
fi
